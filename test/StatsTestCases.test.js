...
const tests = fs
	.readdirSync(base)
	.filter(testName => fs.existsSync(path.join(base, testName, "index.js")) || fs.existsSync(path.join(base, testName, "webpack.config.js")));
	...
	tests.forEach(testName => {
		it("should print correct stats for " + testName, function(done) {
			...
			c.run((err, stats) => {
				...
				if (!hasColorSetting) {
					actual = actual.replace(/\u001b\[[0-9;]*m/g, "").replace(/[0-9]+(\s?ms)/g, "X$1");
				} else {
					actual = actual
