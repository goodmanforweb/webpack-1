				...
				it(testName + " should compile", function(done) {
					...
					webpack(options, (err, stats) => {
							...
							if (Array.isArray(module) || /^\.\.?\//.test(module)) {
								...
								if (options.target === "web" || options.target === "webworker") {
									fn = vm.runInNewContext(
										"(function(require, module, exports, __dirname, __filename, it, window) {" + content + "\n})",
										globalContext,
										p
									);
								} else {
						...
						if (!jsonStats.errors.length && filesCount !== optionsArr.length)
							return done(new Error("Should have found at least one bundle file per webpack config"));
