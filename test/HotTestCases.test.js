...
describe("HotTestCases", () => {
	...
	let categories = fs.readdirSync(casesPath).filter(dir => fs.statSync(path.join(casesPath, dir)).isDirectory());
					...
					compiler.run((err, stats) => {
						...
						function _next(callback) {
							...
							compiler.run((err, stats) => {
								...
								if (checkArrayExpectation(testDirectory, jsonStats, "error", "errors" + fakeUpdateLoaderOptions.updateIndex, "Error", done))
									return;
								if (
						...
						function _require(module) {
							if (module.substr(0, 2) === "./") {
								...
								const fn = vm.runInThisContext(
									"(function(require, module, exports, __dirname, __filename, it, NEXT, STATS) {" + fs.readFileSync(p, "utf-8") + "\n})",
									p
								);
								const m = {
									exports: {}
