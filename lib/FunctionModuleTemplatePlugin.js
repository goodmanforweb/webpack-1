	...
	apply(moduleTemplate) {
		...
		moduleTemplate.plugin("package", (moduleSource, module) => {
			if (moduleTemplate.outputOptions.pathinfo) {
				...
				source.add("  \\****" + req.replace(/./g, "*") + "****/\n");
				if (Array.isArray(module.providedExports) && module.providedExports.length === 0) source.add(Template.toComment("no exports provided") + "\n");
				else if (Array.isArray(module.providedExports)) source.add(Template.toComment("exports provided: " + module.providedExports.join(", ")) + "\n");
				else if (module.providedExports) source.add(Template.toComment("no static exports found") + "\n");
