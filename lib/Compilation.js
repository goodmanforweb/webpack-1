...
class Compilation extends Tapable {
	...
	processModuleDependencies(module, callback) {
		...
		const addDependency = dep => {
			...
			if (resourceIdent) {
				...
				if (factory === undefined) throw new Error(`No module factory available for dependency type: ${dep.constructor.name}`);
	...
	seal(callback) {
		...
		while (
			this.hooks.optimizeChunksBasic.call(this.chunks) ||
			this.hooks.optimizeChunks.call(this.chunks) ||
			this.hooks.optimizeChunksAdvanced.call(this.chunks)
		) {
			/* empty */
		}
		...
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {
			...
			if (shouldRecord) this.hooks.recordModules.call(this.modules, this.records);
	...
	processDependenciesBlocksForChunks(inputChunks) {
		...
		for (const chunk of allCreatedChunks) {
			if (chunk.getNumberOfParents() === 0) chunk.remove("unconnected");
		}
	}
	...
	createHash() {
		...
		Object.keys(this.moduleTemplates)
			.sort()
			.forEach(key => this.moduleTemplates[key].updateHash(hash));
		...
		for (let i = 0; i < this.chunks.length; i++) {
			...
			try {
				...
				for (const fileManifest of manifest) {
					...
					if (this.assets[file] && this.assets[file] !== source) throw new Error(`Conflict: Multiple assets emit to the same filename ${file}`);
	...
	checkConstraints() {
		...
		for (let indexModule = 0; indexModule < modules.length; indexModule++) {
			...
			if (usedIds[moduleId]) throw new Error(`checkConstraints: duplicate module id ${moduleId}`);
		}
