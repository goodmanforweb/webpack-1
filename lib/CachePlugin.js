...
class CachePlugin {
				...
				compiler.plugin("this-compilation", compilation => {
					...
					compilation.plugin("child-compiler", (childCompiler, compilerName, compilerIndex) => {
						if (cache) {
							...
							if (cache.children[compilerName][compilerIndex]) childCache = cache.children[compilerName][compilerIndex];
	...
	applyMtime(mtime) {
		...
		else if (this.FS_ACCURENCY > 100 && mtime % 200 !== 0) this.FS_ACCURENCY = 100;
		else if (this.FS_ACCURENCY > 1000 && mtime % 2000 !== 0) this.FS_ACCURENCY = 1000;
	}
}
