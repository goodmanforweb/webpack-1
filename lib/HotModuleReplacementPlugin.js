	...
	apply(compiler) {
		...
		compiler.plugin("compilation", (compilation, params) => {
				...
				return mainTemplate.asString([
					...
					hotInitCode
						...
						.replace(
							...
							chunk.getNumberOfChunks() > 0 ? "for(var chunkId in installedChunks)" : `var chunkId = ${JSON.stringify(chunk.id)};`
						)
				]);
			});
			...
			normalModuleFactory.plugin(["parser javascript/auto", "parser javascript/dynamic"], (parser, parserOptions) => {
				...
				if (!parser.hooks.hotAcceptCallback) parser.hooks.hotAcceptCallback = new SyncBailHook(["expression", "requests"]);
				if (!parser.hooks.hotAcceptWithoutCallback) parser.hooks.hotAcceptWithoutCallback = new SyncBailHook(["expression", "requests"]);
				parser.plugin("call module.hot.accept", expr => {
					...
					if (expr.arguments.length >= 1) {
						...
						if (params.length > 0) {
							...
							if (expr.arguments.length > 1) parser.hooks.hotAcceptCallback.call(expr.arguments[1], requests);
							else parser.hooks.hotAcceptWithoutCallback.call(expr, requests);
						}
					}
				});
