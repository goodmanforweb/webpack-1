...
const copyWithReason = obj => {
	...
	if (!newObj.reasons || newObj.reasons.indexOf("aggressive-splitted") < 0) newObj.reasons = (newObj.reasons || []).concat("aggressive-splitted");
	return newObj;
};
	...
	apply(compiler) {
		compiler.plugin("this-compilation", compilation => {
			compilation.plugin("optimize-chunks-advanced", chunks => {
				...
				const usedSplits = compilation._aggressiveSplittingSplits ? savedSplits.concat(compilation._aggressiveSplittingSplits) : savedSplits;
						...
						for (let i = 0; i < chunks.length; i++) {
							...
							if (selectedModules.every(m => chunk.containsModule(m))) {
								...
								if (chunk.getNumberOfModules() > splitData.modules.length) {
									...
									if (j < savedSplits.length) newChunk._fromAggressiveSplittingIndex = j;
									...
									if (j < savedSplits.length) chunk._fromAggressiveSplittingIndex = j;
									chunk.name = null;
									if (splitData.id !== null && splitData.id !== undefined) {
				...
				for (let i = 0; i < chunks.length; i++) {
					...
					if (size > maxSize && chunk.getNumberOfModules() > 1) {
						...
						for (let k = 0; k < modules.length; k++) {
							...
							if (chunkSize < maxSize && newSize < maxSize && newSize >= minSize && chunkSize >= minSize) break;
							...
							if (newSize > maxSize || chunkSize < minSize) {
								...
								if (newSize < maxSize && newSize >= minSize && chunkSize >= minSize) break;
							}
						}
