...
const getFinalName = (info, exportName, moduleToInfoMap, requestShortener, asCall, strictHarmonyModule) => {
	switch (info.type) {
		case "concatenated": {
			...
			if (directExport) {
				if (exportName === true) ensureNsObjSource(info, moduleToInfoMap, requestShortener, strictHarmonyModule);
			...
			const problem =
				`Cannot get final name for export "${exportName}" in "${info.module.readableIdentifier(requestShortener)}"` +
...
class ConcatenatedModule extends Module {
	constructor(rootModule, modules) {
		...
		for (const info of this._orderedConcatenationList) {
			if (info.type === "concatenated") {
				...
				m.dependencies.filter(dep => !(dep instanceof HarmonyImportDependency) || !modulesSet.has(dep.module)).forEach(d => this.dependencies.push(d));
				...
				if (m.assets) {
					if (this.assets === undefined) this.assets = Object.create(null);
	...
	get modules() {
		return this._orderedConcatenationList.filter(info => info.type === "concatenated").map(info => info.module);
	}
	...
	source(dependencyTemplates, outputOptions, requestShortener) {
				...
				case "concatenated": {
					...
					info.module.dependencies.forEach(dep => {
						if (dep instanceof HarmonyExportSpecifierDependency) {
							...
							if (!exportMap.has("default")) exportMap.set("default", "__WEBPACK_MODULE_DEFAULT_EXPORT__");
		...
		innerDependencyTemplates.set(
			...
			new HarmonyImportSpecifierDependencyConcatenatedTemplate(dependencyTemplates.get(HarmonyImportSpecifierDependency), moduleToInfoMap)
		);
		innerDependencyTemplates.set(
			HarmonyImportSideEffectDependency,
			new HarmonyImportSideEffectDependencyConcatenatedTemplate(dependencyTemplates.get(HarmonyImportSideEffectDependency), moduleToInfoMap)
		);
		innerDependencyTemplates.set(
			HarmonyExportSpecifierDependency,
			new HarmonyExportSpecifierDependencyConcatenatedTemplate(dependencyTemplates.get(HarmonyExportSpecifierDependency), this.rootModule)
		);
		innerDependencyTemplates.set(
		...
		innerDependencyTemplates.set(
			...
			new HarmonyCompatibilityDependencyConcatenatedTemplate(dependencyTemplates.get(HarmonyCompatibilityDependency), this.rootModule, moduleToInfoMap)
		);
		...
		if (dep.originModule === this.rootModule) {
			if (this.modulesMap.get(dep.module)) {
				...
				exportDefs.forEach(def => {
					...
					const content = `/* concated harmony reexport */__webpack_require__.d(${exportsName}, ${JSON.stringify(used)}, function() { return ${
						finalName
					}; });\n`;
