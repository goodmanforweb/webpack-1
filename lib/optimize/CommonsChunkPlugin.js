		...
		compiler.plugin("this-compilation", compilation => {
			compilation.plugin(["optimize-chunks", "optimize-extracted-chunks"], chunks => {
				...
				targetChunks.forEach((targetChunk, idx) => {
						...
						if (!asyncChunk) {
							asyncChunk = this.createAsyncChunk(
								...
								targetChunks.length <= 1 || typeof this.async !== "string"
									? this.async
									: targetChunk.name ? `${this.async}-${targetChunk.name}` : true,
								targetChunk
							);
						}
					...
					if (this.filenameTemplate) targetChunk.filenameTemplate = this.filenameTemplate;
	...
	getAffectedChunks(compilation, allChunks, targetChunk, targetChunks, currentIndex, selectedChunks, asyncOption, childrenOption, deepChildrenOption) {
		...
		if (targetChunk.getNumberOfParents() > 0) {
			compilation.errors.push(
				new Error("CommonsChunkPlugin: While running in normal mode it's not allowed to use a non-entry chunk (" + targetChunk.name + ")")
			);
			return;
		}
