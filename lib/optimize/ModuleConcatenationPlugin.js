					...
					if (nonHarmonyReasons.length > 0) {
						...
						setBailoutReason(module, requestShortener => {
							const names = Array.from(importingModules)
								.map(m => `${m.readableIdentifier(requestShortener)} (referenced with ${Array.from(importingModuleTypes.get(m)).join(", ")})`)
								.sort();
				...
				for (const currentRoot of relevantModules) {
					...
					if (!currentConfiguration.isEmpty()) {
						...
						for (const module of currentConfiguration.getModules()) {
							if (module !== currentConfiguration.rootModule) usedAsInner.add(module);
						}
					}
				}
						...
						newModule.optimizationBailout.push(requestShortener => {
							...
							if (warning[0] === warning[1])
								...
								return formatBailoutReason(
									`Cannot concat with ${warning[0].readableIdentifier(requestShortener)} because of ${warning[1].readableIdentifier(
										requestShortener
									)}${reasonWithPrefix}`
								);
						});
