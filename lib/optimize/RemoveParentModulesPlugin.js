		...
		compiler.plugin("compilation", compilation => {
			compilation.plugin(["optimize-chunks-basic", "optimize-extracted-chunks-basic"], chunks => {
				...
				for (const chunk of chunks) {
					...
					if (chunk.getNumberOfParents() === 0) availableModulesMap.set(chunk, new Set());
				}
				...
				while (queue.length > 0) {
					...
					for (const parent of chunk.parentsIterable) {
						...
						if (availableModulesInParent !== undefined) {
							...
							if (availableModules === undefined) {
								...
								for (const m of parent.modulesIterable) availableModules.add(m);
					...
					if (changed) {
						...
						for (const child of chunk.chunksIterable) queue.enqueue(child);
					}
				}
				...
				for (const chunk of chunks) {
					...
					if (modules.size < availableModules.size) {
						for (const m of modules) if (availableModules.has(m)) toRemove.add(m);
					} else {
						for (const m of availableModules) if (modules.has(m)) toRemove.add(m);
					}
