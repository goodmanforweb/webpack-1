...
class Stats {
	...
	toJson(options, forToString) {
		...
		const optionOrLocalFallback = (v, def) => (typeof v !== "undefined" ? v : typeof options.all !== "undefined" ? options.all : def);
		...
		const testAgainstGivenOption = item => {
			...
			if (item && typeof item === "object" && typeof item.test === "function") return ident => item.test(ident);
		...
		if (showChildren) {
			obj.children = compilation.children.map((child, idx) => {
				...
				delete obj.hash;
				delete obj.version;
				if (child.name) obj.name = identifierUtils.makePathsRelative(context, child.name, compilation.cache);
				return obj;
			});
		}
		...
		const processModuleContent = (module, prefix) => {
			...
			if (module.usedExports !== undefined) {
				if (module.usedExports !== true) {
					colors.normal(prefix);
					if (module.usedExports === false || module.usedExports.length === 0) colors.cyan("[no exports used]");
	...
	static getChildOptions(options, idx) {
		...
		if (typeof innerOptions === "boolean" || typeof innerOptions === "string") innerOptions = Stats.presetToOptions(innerOptions);
