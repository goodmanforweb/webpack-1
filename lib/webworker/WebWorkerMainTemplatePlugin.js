	...
	apply(mainTemplate) {
		mainTemplate.plugin("local-vars", (source, chunk) => {
			if (chunk.getNumberOfChunks() > 0) {
				return mainTemplate.asString([
					...
					"var installedChunks = {",
					mainTemplate.indent(chunk.ids.map(id => `${id}: 1`).join(",\n")),
					"};"
				]);
			}
			return source;
		...
		mainTemplate.plugin("bootstrap", (source, chunk, hash) => {
			if (chunk.getNumberOfChunks() > 0) {
				const chunkCallbackName =
					mainTemplate.outputOptions.chunkCallbackName || Template.toIdentifier("webpackChunk" + (mainTemplate.outputOptions.library || ""));
				return mainTemplate.asString([
					source,
		...
		mainTemplate.plugin("hot-bootstrap", (source, chunk, hash) => {
			...
			const hotUpdateFunction =
				mainTemplate.outputOptions.hotUpdateFunction || Template.toIdentifier("webpackHotUpdate" + (mainTemplate.outputOptions.library || ""));
