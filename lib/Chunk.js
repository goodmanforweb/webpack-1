...
class Chunk {
	...
	setBlocks(newBlocks) {
		this._blocks.clear();
		for (const p of newBlocks) this._blocks.add(p);
	}
	...
	setEntrypoints(newEntrypoints) {
		...
		for (const p of newEntrypoints) this._entrypoints.add(p);
	}
	...
	hasModuleInGraph(filterFn) {
		...
		while (queue.length > 0) {
			const chunk = queue.pop();
			for (const module of chunk.modulesIterable) if (filterFn(module)) return true;
	...
	checkConstraints() {
		...
		for (const child of chunk._chunks) {
			if (!child._parents.has(chunk)) throw new Error(`checkConstraints: child missing parent ${chunk.debugId} -> ${child.debugId}`);
		}
		for (const parentChunk of chunk._parents) {
			if (!parentChunk._chunks.has(chunk)) throw new Error(`checkConstraints: parent missing child ${parentChunk.debugId} <- ${chunk.debugId}`);
		}
	}
}
