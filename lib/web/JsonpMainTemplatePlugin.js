...
class JsonpMainTemplatePlugin {
	apply(mainTemplate) {
		...
		// TODO refactor this
		if (!mainTemplate.hooks.jsonpScript) mainTemplate.hooks.jsonpScript = new SyncWaterfallHook(["source", "chunk", "hash"]);
		mainTemplate.plugin("local-vars", (source, chunk) => {
			if (needChunkLoadingCode(chunk)) {
				return mainTemplate.asString([
					...
					"var installedChunks = {",
					mainTemplate.indent(chunk.ids.map(id => `${JSON.stringify(id)}: 0`).join(",\n")),
					"};",
					"",
		...
		mainTemplate.plugin("jsonp-script", (_, chunk, hash) => {
			...
			return mainTemplate.asString([
				...
				mainTemplate.indent(["onScriptComplete({ type: 'timeout', target: script });"]),
				`}, ${chunkLoadTimeout});`,
		...
		mainTemplate.plugin("require-ensure", (source, chunk, hash) => {
			return mainTemplate.asString([
				...
				mainTemplate.indent([
					...
					mainTemplate.indent(["promises.push(installedChunkData[2]);"]),
					"} else {",
					mainTemplate.indent([
						...
						mainTemplate.entryPointInChildren(chunk)
							? [
									...
									mainTemplate.indent([
										...
										mainTemplate.indent(["var depId = scheduledModule[j];", "if(installedChunks[depId] !== 0) fullfilled = false;"]),
										"}",
										"if(fullfilled) {",
										mainTemplate.indent([
