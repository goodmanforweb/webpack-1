...
class FetchCompileWasmMainTemplatePlugin {
	apply(mainTemplate) {
		mainTemplate.plugin("local-vars", (source, chunk) => {
			if (!chunk.hasModuleInGraph(m => m.type.startsWith("webassembly"))) return source;
			return mainTemplate.asString([source, "", "// object to store loaded and loading wasm modules", "var installedWasmModules = {};"]);
		});
				...
				mainTemplate.indent([
					...
					mainTemplate.indent([
						...
						mainTemplate.indent([
							...
							mainTemplate.indent(["return WebAssembly.compileStreaming(response);"]),
							"} else {",
							mainTemplate.indent(["return response.arrayBuffer().then(function(bytes) { return WebAssembly.compile(bytes); });"]),
							"}"
						]),
		...
		mainTemplate.plugin("require-extensions", (source, chunk) => {
			if (!chunk.hasModuleInGraph(m => m.type.startsWith("webassembly"))) return source;
			return mainTemplate.asString([source, "", "// object with all compiled WebAssmbly.Modules", `${mainTemplate.requireFn}.w = {};`]);
		});
