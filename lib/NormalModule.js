...
class NormalModule extends Module {
	...
	createLoaderContext(resolver, options, compilation, fs) {
		const loaderContext = {
			version: 2,
			emitWarning: warning => {
				if (!(warning instanceof Error)) warning = new NonErrorEmittedError(warning);
	...
	sourceBlock(block, availableVars, dependencyTemplates, source, outputOptions, requestShortener) {
		...
		const vars = block.variables.reduce((result, value) => {
			const variable = this.sourceVariables(value, availableVars, dependencyTemplates, outputOptions, requestShortener);
		...
		if (vars.length > 0) {
			...
			const functionWrapperEnds = injectionVariableChunks.map(variableChunk => {
				return this.variableInjectionFunctionWrapperEndCode(variableChunk.map(variable => variable.expression), block);
			});
