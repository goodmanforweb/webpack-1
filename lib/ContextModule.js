...
class ContextModule extends Module {
	...
	constructor(resolveDependencies, options) {
		...
		if (options.resolveOptions !== undefined) this.resolveOptions = options.resolveOptions;

		// Info from Build
		this.builtTime = undefined;
		this.contextDependencies = new Set([this.context]);

		if (typeof options.mode !== "string") throw new Error("options.mode is a required option");
	}
	...
	identifier() {
		...
		if (this.options.namespaceObject === "strict") identifier += " strict namespace object";
	...
	readableIdentifier(requestShortener) {
		...
		if (this.options.mode) identifier += ` ${this.options.mode}`;
		if (!this.options.recursive) identifier += " nonrecursive";
		if (this.options.addon) identifier += ` ${requestShortener.shorten(this.options.addon)}`;
		if (this.options.regExp) identifier += ` ${this.prettyRegExp(this.options.regExp + "")}`;
		if (this.options.include) identifier += ` include: ${this.prettyRegExp(this.options.include + "")}`;
		if (this.options.exclude) identifier += ` exclude: ${this.prettyRegExp(this.options.exclude + "")}`;
		if (this.options.namespaceObject === "strict") identifier += " strict namespace object";
		else if (this.options.namespaceObject) identifier += " namespace object";
	...
	libIdent(options) {
		...
		if (this.options.addon) identifier += ` ${this.contextify(options.context, this.options.addon)}`;
		if (this.options.regExp) identifier += ` ${this.prettyRegExp(this.options.regExp + "")}`;
		if (this.options.include) identifier += ` include: ${this.prettyRegExp(this.options.include + "")}`;
		if (this.options.exclude) identifier += ` exclude: ${this.prettyRegExp(this.options.exclude + "")}`;
	...
	getModuleObjectSource(fakeMap, fakeMapDataExpression = "fakeMap[id]") {
		...
		if (strict) {
			...
			if (fakeMap)
				...
				} || typeof module === "object" && module && module.__esModule ? module : /* fake namespace object */ { "default": module };`;
		}
		return "module";
	}
	...
	getLazySource(blocks, id) {
		...
		const map = blocks
			...
			.reduce((map, item) => {
				...
				if (typeof fakeMap === "object") arrayStart.push(fakeMap[item.dependency.module.id]);
...
webpackEmptyAsyncContext.id = ${JSON.stringify(id)};`;
	...
	source(dependencyTemplates, outputOptions, requestShortener) {
		return this.getSource(this.getSourceString(this.options.mode, outputOptions, requestShortener));
	}
