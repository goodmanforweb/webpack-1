...
class WebpackOptionsValidationError extends WebpackError {
	...
	static formatSchema(schema, prevSchemas) {
		...
		if (schema.type === "string") {
			...
			else if (schema.minLength > 1) return `string (min length ${schema.minLength})`;
			return "string";
	...
	static formatValidationError(err) {
		...
		if (err.keyword === "additionalProperties") {
			...
			if (err.children && err.children.length > 0) {
				...
				return (
					...
					`Details:\n${filterChildren(err.children)
						.map(err => " * " + indent(WebpackOptionsValidationError.formatValidationError(err), "   ", false))
						.join("\n")}`
				);
			}
			...
			if (err.params.limit === 1) return `${dataPath} should not be empty.${getSchemaPartDescription(err.parentSchema)}`;
			else return `${dataPath} ${err.message}${getSchemaPartDescription(err.parentSchema)}`;
