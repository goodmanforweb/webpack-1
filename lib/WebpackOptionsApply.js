...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		compiler.apply(new JavascriptModulesPlugin(), new JsonModulesPlugin(), new WebAssemblyModulesPlugin());
		...
		if (typeof options.mode !== "string") compiler.apply(new WarnNoModeSetPlugin());

		compiler.apply(new EnsureChunkConditionsPlugin());
		if (options.optimization.removeAvailableModules) compiler.apply(new RemoveParentModulesPlugin());
		if (options.optimization.removeEmptyChunks) compiler.apply(new RemoveEmptyChunksPlugin());
		if (options.optimization.mergedDuplicateChunks) compiler.apply(new MergeDuplicateChunksPlugin());
		if (options.optimization.flagIncludedChunks) compiler.apply(new FlagIncludedChunksPlugin());
		if (options.optimization.occurrenceOrder) compiler.apply(new OccurrenceOrderPlugin(true));
		if (options.optimization.sideEffects) compiler.apply(new SideEffectsFlagPlugin());
		if (options.optimization.providedExports) compiler.apply(new FlagDependencyExportsPlugin());
		if (options.optimization.usedExports) compiler.apply(new FlagDependencyUsagePlugin());
		if (options.optimization.concatenateModules) compiler.apply(new ModuleConcatenationPlugin());
		if (options.optimization.noEmitOnErrors) compiler.apply(new NoEmitOnErrorsPlugin());
		if (options.optimization.namedModules) compiler.apply(new NamedModulesPlugin());
		if (options.optimization.namedChunks) compiler.apply(new NamedChunksPlugin());
