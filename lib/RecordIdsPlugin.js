	...
	apply(compiler) {
		...
		compiler.plugin("compilation", compilation => {
			compilation.plugin("record-modules", (modules, records) => {
				...
				modules.forEach(module => {
					const identifier = portableIds
						? identifierUtils.makePathsRelative(compiler.context, module.identifier(), compilation.cache)
						: module.identifier();
			...
			const getDepBlockIdent = (chunk, block) => {
				...
				const identifier = portableIds
					? identifierUtils.makePathsRelative(compiler.context, block.identifier(), compilation.cache)
					: block.identifier();
