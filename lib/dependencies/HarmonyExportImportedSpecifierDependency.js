	...
	_discoverActiveExportsFromOtherStartExports() {
		...
		for (const otherStarExport of this.otherStarExports) {
			...
			if (otherImportedModule && Array.isArray(otherImportedModule.providedExports)) {
				for (const exportName of otherImportedModule.providedExports) result.add(exportName);
			}
		}
		return result;
	}
...
HarmonyExportImportedSpecifierDependency.Template = class HarmonyExportImportedSpecifierDependencyTemplate extends HarmonyImportDependency.Template {
		...
		switch (mode.type) {
			...
			case "reexport-fake-namespace-object":
				return (
					...
					this.getReexportFakeNamespaceObjectStatement(module, module.isUsed(mode.name), importVar)
				);

			case "rexport-non-harmony-undefined":
				return (
					"/* harmony reexport (non default export from non-hamory) */ " +
					this.getReexportStatement(module, module.isUsed(mode.name), "undefined", "")
				);
				...
				return Array.from(mode.map.entries())
					.map(item => {
						return (
							...
							this.getReexportStatement(module, module.isUsed(item[0]), importVar, importedModule.isUsed(item[1])) +
							"\n"
						);
					})
					.join("");
			...
			case "dynamic-reexport": {
				...
				if (activeExports.length > 0) content += "if(" + JSON.stringify(activeExports.concat("default")) + ".indexOf(__WEBPACK_IMPORT_KEY__) < 0) ";
				else content += "if(__WEBPACK_IMPORT_KEY__ !== 'default') ";
				const exportsName = dep.originModule.exportsArgument;
				return (
					content + `(function(key) { __webpack_require__.d(${exportsName}, key, function() { return ${name}[key]; }) }(__WEBPACK_IMPORT_KEY__));\n`
				);
			}
	...
	getConditionalReexportStatement(module, key, name, valueKey) {
		const exportsName = module.exportsArgument;
		const returnValue = this.getReturnValue(valueKey);
		return `if(__webpack_require__.o(${name}, ${JSON.stringify(valueKey)})) __webpack_require__.d(${exportsName}, ${JSON.stringify(
			key
