	...
	getContent(dep, importedVar) {
		...
		if (nonHarmonyImport) {
			...
			if (dep.originModule.meta.strictHarmonyModule) {
				...
				if (!dep.id) {
					if (shortHandPrefix) return `${shortHandPrefix}/* fake namespace object for non-esm import */ { default: ${importedVar} }`;
					else return `Object(/* fake namespace object for non-esm import */{ "default": ${importedVar} })`;
				}
